...
int main()
{
  ...
  auto modelLoader = ModelLoader<Vertex>(1);
  ...
  Shader modelShader(getExecutablePath() + "/mesh.vert", getExecutablePath() + "/mesh.frag");
  ...
  modelShader.setMatrix4fv("view", cam.GetViewMatrix());
  modelShader.setMatrix4fv("projection", app.getProjectionMatrix());
  ...
  lampShader.use();
  lampShader.setMatrix4fv("model", glm::mat4(1.0f));
  ...
  app.run([=, &lampShader, &modelShader, &modelLoader, &cam, &app, &dm](float a_delta){
