...
int main()
{
  ...
  Shader ourShader(getExecutablePath() + "/shader.vs", getExecutablePath() + "/shader.fs");
  ...
  while (!glfwWindowShouldClose(window))
  {
      ...
      // the sequence of the transform does matters!
      ...
      trans = glm::mat4(1.0f);
      // the sequence of the transform does matters!
      float scale = std::sin((float)glfwGetTime()) / 2.0f + 0.5f;
      trans = glm::scale(trans, glm::vec3(scale, scale, scale));
      trans = glm::translate(trans, glm::vec3(-(1-scale/2)/scale, (1-scale/2)/scale, 0.0f));
