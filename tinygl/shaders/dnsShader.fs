...
// for static light we don't care about the position
struct DirectLight {
  vec3 direction;
  // the spec of the light
  vec3 ambient;
  vec3 diffuse;
  vec3 specular;
};
...
// for each light, we need to consider 3 different light modes: ambient, diffuse, specular
// following code use a lot of dot product, which is using the cos feature of the dot product: the larger the angle the smaller the value and generate a scalar
// remember always need to normalize before use direction vector
...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
  ...
  vec3 ambient  = light.ambient  * vec3(texture(texture_diffuse0, TexCoord));
  vec3 diffuse  = light.diffuse  * diff * vec3(texture(texture_diffuse0, TexCoord));
  // if need emission check the multiplelight example
  vec3 specular = light.specular * spec * vec3(texture(texture_specular0, TexCoord));
  ambient  *= attenuation;
  diffuse  *= attenuation;
